...
export function ReasonPromptDialog({ title, description, confirmLabel, isSubmitting, onClose, onConfirm }: ReasonPromptDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" role="presentation" onMouseDown={(event) => event.stopPropagation()}>
      <section aria-labelledby="reason-prompt-title" aria-modal="true" className="max-h-[85dvh] w-full max-w-md overflow-y-auto rounded-card border border-border bg-surface p-6 shadow-panel sm:p-8" role="dialog">
        <div className="flex items-start justify-between gap-4">
          <h2 id="reason-prompt-title" className="text-lg font-bold text-ink">{title}</h2>
