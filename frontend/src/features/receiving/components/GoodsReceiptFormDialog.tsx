...
export function GoodsReceiptFormDialog({ receivablePurchaseOrders, isSaving, onClose, onSave }: GoodsReceiptFormDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" role="presentation">
      <section aria-labelledby="gr-form-title" aria-modal="true" className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl border border-border bg-surface p-6 shadow-panel" role="dialog">
        ...
        <form className="mt-6 space-y-5" onSubmit={submit}>
          <div className="grid gap-4 sm:grid-cols-3">
            <label className="text-sm font-semibold text-ink sm:col-span-2">Purchase order
              <select className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" required value={purchaseOrderId} onChange={(event) => setPurchaseOrderId(event.target.value)}>
            ...
            <label className="text-sm font-semibold text-ink">Received date<input className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" required type="date" value={receivedAt} onChange={(event) => setReceivedAt(event.target.value)} /></label>
          </div>
          <label className="block text-sm font-semibold text-ink sm:max-w-sm">Supplier delivery reference<input className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" value={supplierDeliveryNumber} onChange={(event) => setSupplierDeliveryNumber(event.target.value)} /></label>
            ...
            <div>
              ...
              <div className="mt-2 space-y-3">
                {lines.map((line, index) => (
                  <div className="rounded-lg border border-border p-3" key={line.purchaseOrderLineId}>
                    ...
                    <div className="mt-2 grid grid-cols-3 gap-2">
                      <label className="text-xs font-semibold text-muted">Received
                        <input className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600" min="0" required step="0.0001" type="number" value={line.receivedQuantity} onChange={(event) => updateLine(index, { receivedQuantity: event.target.value })} />
                      </label>
                      <label className="text-xs font-semibold text-muted">Accepted
                        <input className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600" min="0" required step="0.0001" type="number" value={line.acceptedQuantity} onChange={(event) => updateLine(index, { acceptedQuantity: event.target.value })} />
                      </label>
                      <label className="text-xs font-semibold text-muted">Rejected
                        <input className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600" min="0" required step="0.0001" type="number" value={line.rejectedQuantity} onChange={(event) => updateLine(index, { rejectedQuantity: event.target.value })} />
                      </label>
                    </div>
                    {Number(line.rejectedQuantity) > 0 ? (
                      <label className="mt-2 block text-xs font-semibold text-muted">Rejection reason
                        <input className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600" required value={line.rejectionReason} onChange={(event) => updateLine(index, { rejectionReason: event.target.value })} />
                      </label>
                    ) : null}
                  </div>
                ))}
          ...
          <label className="block text-sm font-semibold text-ink">Notes<textarea className="mt-2 w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" rows={2} value={notes} onChange={(event) => setNotes(event.target.value)} /></label>
