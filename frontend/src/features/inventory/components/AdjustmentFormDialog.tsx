          ...
          <div className={cn(sheetBodyClass, 'space-y-5')}>
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="text-sm font-semibold text-ink">Reason
                <select className="mt-2 h-11 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" required value={values.reasonCode} onChange={(event) => setValues((state) => ({ ...state, reasonCode: event.target.value as AdjustmentFormValues['reasonCode'] }))}>
