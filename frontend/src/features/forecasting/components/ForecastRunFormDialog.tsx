...
export function ForecastRunFormDialog({ isSaving, onClose, onSave }: ForecastRunFormDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" role="presentation">
      <section aria-labelledby="forecast-run-form-title" aria-modal="true" className="w-full max-w-lg rounded-xl border border-border bg-surface p-6 shadow-panel" role="dialog">
        ...
        <form className="mt-6 grid gap-4" onSubmit={submit}>
          <label className="text-sm font-semibold text-ink">Period grain
            <select className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" value={periodGrain} onChange={(event) => setPeriodGrain(event.target.value as PeriodGrain)}>
          ...
          <label className="text-sm font-semibold text-ink">Window periods (2–24)
            <input className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" max={24} min={2} required type="number" value={windowPeriods} onChange={(event) => setWindowPeriods(Number(event.target.value))} />
          </label>
          <label className="text-sm font-semibold text-ink">History end date
            <input className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" required type="date" value={historyEndDate} onChange={(event) => setHistoryEndDate(event.target.value)} />
          </label>
