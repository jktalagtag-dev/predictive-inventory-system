...
export function CheckoutSummary({ totals, paymentsTotal, isValid, isSubmitting, disabledReason, onFinalize }: CheckoutSummaryProps) {
  ...
  return (
    <section aria-label="Checkout summary" className="space-y-3 rounded-xl border border-border bg-surface p-4 shadow-panel">
      <dl className="space-y-1.5 text-sm">
        ...
        <div className={`flex justify-between ${paymentMismatch ? 'font-semibold text-red-700' : 'text-muted'}`}><dt>Payments entered</dt><dd className="tabular-nums">{paymentsTotal.toFixed(2)}</dd></div>
      </dl>
