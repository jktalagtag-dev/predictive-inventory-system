...
export function ReorderPolicyDetailsDrawer({ policy, canCalculate, canCalculateEoq, isActing, onClose, onRecalculateRop, onCalculateEoq }: ReorderPolicyDetailsDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-slate-950/30" role="presentation" onMouseDown={onClose}>
      <aside aria-labelledby="rop-details-title" aria-modal="true" className="ml-auto flex h-full w-full max-w-2xl flex-col border-l border-border bg-surface shadow-panel" role="dialog" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-6">
          ...
          {canCalculateEoq ? (
            <section className="space-y-3 rounded-lg border border-border p-4">
              ...
              <form className="grid grid-cols-3 gap-3" onSubmit={submitEoq}>
                <label className="text-xs font-semibold text-muted">Annual demand
                  <input className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600 focus:ring-1 focus:ring-brand-600/30" min="0" required step="0.01" type="number" value={annualDemand} onChange={(event) => setAnnualDemand(event.target.value)} />
                </label>
                <label className="text-xs font-semibold text-muted">Ordering cost
                  <input className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600 focus:ring-1 focus:ring-brand-600/30" min="0" required step="0.01" type="number" value={orderingCost} onChange={(event) => setOrderingCost(event.target.value)} />
                </label>
                <label className="text-xs font-semibold text-muted">Holding cost / unit
                  <input className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600 focus:ring-1 focus:ring-brand-600/30" min="0" required step="0.01" type="number" value={holdingCost} onChange={(event) => setHoldingCost(event.target.value)} />
                </label>
                <div className="col-span-3"><Button disabled={isActing} type="submit">Calculate EOQ</Button></div>
              </form>

              {eoqHistoryQuery.data && eoqHistoryQuery.data.length > 0 ? (
                <ul className="space-y-1.5 text-sm">
                  {eoqHistoryQuery.data.map((calc) => (
                    <li key={calc.id} className="flex justify-between rounded-md border border-border px-3 py-2">
