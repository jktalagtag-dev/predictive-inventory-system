...
export function ReorderPolicyFormDialog({ productOptions, isSaving, onClose, onSave }: ReorderPolicyFormDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" role="presentation">
      <section aria-labelledby="reorder-policy-form-title" aria-modal="true" className="w-full max-w-lg rounded-xl border border-border bg-surface p-6 shadow-panel" role="dialog">
        ...
        <form className="mt-6 grid gap-4" onSubmit={submit}>
          <label className="text-sm font-semibold text-ink">Product
            <select className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" required value={productId} onChange={(event) => setProductId(event.target.value)}>
          ...
          <label className="text-sm font-semibold text-ink">Safety stock quantity
            <input className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" min="0" required step="0.0001" type="number" value={safetyStockQuantity} onChange={(event) => setSafetyStockQuantity(event.target.value)} />
          </label>
          <label className="text-sm font-semibold text-ink">Safety stock basis
            <select className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" value={safetyStockBasis} onChange={(event) => setSafetyStockBasis(event.target.value as SafetyStockBasis)}>
