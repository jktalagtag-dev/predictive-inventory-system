...
export function ReasonPromptDialog({ title, description, confirmLabel, isSubmitting, onClose, onConfirm }: ReasonPromptDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" role="presentation" onMouseDown={(event) => event.stopPropagation()}>
      <section aria-labelledby="restocking-reason-prompt-title" aria-modal="true" className="max-h-[85dvh] w-full max-w-md overflow-y-auto rounded-card border border-border bg-surface p-6 shadow-panel sm:p-8" role="dialog">
        <div className="flex items-start justify-between gap-4">
          <h2 id="restocking-reason-prompt-title" className="text-lg font-bold text-ink">{title}</h2>
        ...
        <form className="mt-4 space-y-4" onSubmit={submit}>
          <label className="block text-sm font-semibold text-ink">Reason
            <textarea autoFocus className="mt-2 w-full rounded-xl border border-border bg-surface px-3 py-2 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" required rows={3} value={reason} onChange={(event) => setReason(event.target.value)} />
          </label>
