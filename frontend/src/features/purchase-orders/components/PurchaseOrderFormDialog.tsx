...
export function PurchaseOrderFormDialog({ supplierOptions, productOptions, unitOptions, isSaving, onClose, onSave }: PurchaseOrderFormDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" role="presentation">
      <section aria-labelledby="po-form-title" aria-modal="true" className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl border border-border bg-surface p-6 shadow-panel" role="dialog">
        <div className="flex items-start justify-between gap-4"><div><h2 id="po-form-title" className="text-lg font-bold text-ink">Create purchase order</h2><p className="mt-1 text-sm text-muted">Totals are calculated by the server once the draft is created.</p></div><Button aria-label="Close dialog" size="icon" variant="ghost" onClick={onClose}><X aria-hidden="true" size={18} /></Button></div>
        <form className="mt-6 space-y-5" onSubmit={submit}>
          <div className="grid gap-4 sm:grid-cols-3">
            <label className="text-sm font-semibold text-ink sm:col-span-1">Supplier
              <select className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" required value={values.supplierId} onChange={(event) => {
            ...
            <label className="text-sm font-semibold text-ink">Currency<input className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm uppercase outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" maxLength={3} required value={values.currencyCode} onChange={(event) => setValues((state) => ({ ...state, currencyCode: event.target.value.toUpperCase() }))} /></label>
            <label className="text-sm font-semibold text-ink">Expected receipt<input className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" type="date" value={values.expectedReceiptAt} onChange={(event) => setValues((state) => ({ ...state, expectedReceiptAt: event.target.value }))} /></label>
          </div>
          ...
          <div>
            ...
            <div className="mt-2 space-y-3">
              {values.lines.map((line, index) => (
                <div className="grid grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_90px_100px_80px_80px_36px] items-end gap-2 rounded-lg border border-border p-3" key={index}>
                  ...
                  <label className="text-xs font-semibold text-muted">Unit
                    <select className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600" required value={line.unitId} onChange={(event) => updateLine(index, { unitId: event.target.value })}>
                      <option value="" disabled>Select</option>
                  ...
                  <label className="text-xs font-semibold text-muted">Qty
                    <input className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600" min="0" required step="0.0001" type="number" value={line.orderedQuantity} onChange={(event) => updateLine(index, { orderedQuantity: event.target.value })} />
                  </label>
                  <label className="text-xs font-semibold text-muted">Unit cost
                    <input className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600" min="0" required step="0.0001" type="number" value={line.unitCost} onChange={(event) => updateLine(index, { unitCost: event.target.value })} />
                  </label>
                  <label className="text-xs font-semibold text-muted">Tax %
                    <input className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600" min="0" step="0.0001" type="number" value={line.taxRate} onChange={(event) => updateLine(index, { taxRate: event.target.value })} />
                  </label>
                  <label className="text-xs font-semibold text-muted">Discount
                    <input className="mt-1 h-9 w-full rounded-md border border-border bg-surface px-2 text-sm outline-none focus:border-brand-600" min="0" step="0.0001" type="number" value={line.discountAmount} onChange={(event) => updateLine(index, { discountAmount: event.target.value })} />
                  </label>
          ...
          <label className="block text-sm font-semibold text-ink">Notes
            <textarea className="mt-2 w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20" rows={2} value={values.notes} onChange={(event) => setValues((state) => ({ ...state, notes: event.target.value }))} />
          </label>
