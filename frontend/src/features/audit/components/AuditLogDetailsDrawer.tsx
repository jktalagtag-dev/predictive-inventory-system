...
export function AuditLogDetailsDrawer({ entry, onClose }: { entry: AuditLogEntry; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-40 bg-slate-950/30" role="presentation" onMouseDown={onClose}>
      <aside aria-labelledby="audit-details-title" aria-modal="true" className="ml-auto flex h-full w-full max-w-xl flex-col border-l border-border bg-surface shadow-panel" role="dialog" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-6">
          <section className="grid grid-cols-2 gap-4 rounded-lg border border-border p-4 text-sm">
            ...
            <div className="col-span-2"><p className="text-muted">Correlation ID</p><p className="font-mono text-xs font-semibold text-ink">{entry.correlationId}</p></div>
          </section>
            ...
            <section className="space-y-3">
              ...
              {entry.changes.before ? (
                <div>
                  ...
                  <pre className="mt-1 overflow-x-auto rounded-lg border border-border bg-subtle p-3 text-xs">{JSON.stringify(entry.changes.before, null, 2)}</pre>
                </div>
              ) : null}
              {entry.changes.after ? (
                <div>
                  <p className="text-xs font-semibold text-muted">After</p>
                  <pre className="mt-1 overflow-x-auto rounded-lg border border-border bg-subtle p-3 text-xs">{JSON.stringify(entry.changes.after, null, 2)}</pre>
                </div>
              ) : null}
            </section>
